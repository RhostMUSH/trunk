...
POWENT pow_table[] =
{
  ...
  {"ANSINAME", POWER_ANSINAME, 'a', POWER5, 0, POWER_LEVEL_NA, pw_wiz},
...
POWENT depow_table[] =
{
  ...
  {"ANSINAME", DP_ANSINAME, 'A', POWER8, 0, POWER_LEVEL_NA, pw_wiz2},
...
NDECL(init_flagtab)
{
	...
	*np = ToLower((int)*bp);
      ...
      *np = '\0';
      ohtab_add(nbuf, (int *) fp, &mudstate_hot.flags_htab, 1);
    }
    free_sbuf(nbuf);
...
NDECL(init_toggletab)
{
    ...
    ohtab_init(&mudstate_hot.toggles_htab, 131);
	...
	*np = '\0';
	ohtab_add(nbuf, (int *) tp, &mudstate_hot.toggles_htab, 0);
    }
    free_sbuf(nbuf);
}
...
void display_flagtab2(dbref player, char *buff, char **bufcx)
{
    ...
    for (fp = (FLAGENT *) ohtab_firstentry2(&mudstate_hot.flags_htab, 1); 
	 fp;
	 fp = (FLAGENT *) ohtab_nextentry(&mudstate_hot.flags_htab)) {
...
display_toggletab2(dbref player, char *buff, char **bufcx)
{
    ...
    for (tp = (TOGENT *) ohtab_firstentry(&mudstate_hot.toggles_htab);
         tp;
	 tp = (TOGENT *) ohtab_nextentry(&mudstate_hot.toggles_htab)) {
...
find_flag_perm(dbref thing, char *flagname, dbref player)
{
	...
	*cp = ToLower((int)*cp);
    ...
    fp = (FLAGENT *)ohtab_find(flagname, &mudstate_hot.flags_htab);
    if ( fp ) {
       if ((fp->listperm & CA_BUILDER) && !Builder(player))
          fp = (FLAGENT*)NULL;
       else if ((fp->listperm & CA_ADMIN) && !Admin(player))
...
find_flag(dbref thing, char *flagname)
{
    ...
    for (cp = flagname; *cp; cp++)
	*cp = ToLower((int)*cp);
    return (FLAGENT *) ohtab_find(flagname, &mudstate_hot.flags_htab);
}
...
find_toggle_perm(dbref thing, char *togglename, dbref player)
{
	...
	*cp = ToLower((int)*cp);
    ...
    tp = (TOGENT *)ohtab_find(togglename, &mudstate_hot.toggles_htab);
...
find_toggle(dbref thing, char *togglename)
{
    ...
    for (cp = togglename; *cp; cp++)
	*cp = ToLower((int)*cp);
    return (TOGENT *) ohtab_find(togglename, &mudstate_hot.toggles_htab);
}
...
int flagstuff_internal(char *alias, char *newname)
{
  ...
  *pt4 = '\0';
  strcpy(buff3, buff1);
  flag1 = (FLAGENT *)ohtab_find(buff1, &mudstate_hot.flags_htab);
  if (flag1 != NULL) {
    flag2 = (FLAGENT *)ohtab_find(buff2, &mudstate_hot.flags_htab);
    if (flag2 == NULL) {
	...
	*pt3++ = toupper(*pt1++);
      ...
      *pt2 = '\0';
      *pt3 = '\0';
      ohtab_repl(buff3, (int *) flag1, &mudstate_hot.flags_htab, 0);
      strcpy(flag1->flagname,buff2);
      ohtab_add(buff1, (int *) flag1, &mudstate_hot.flags_htab, 1);
      if (mudstate_hot.initializing) {
...
int do_flag_and_toggle_def_conf(dbref player, char *totem_perms, char *cmd, char *totem, int i_type ) {
   ...
   if ( i_type == IS_TYPE_FLAG ) {
      fp = (FLAGENT *)ohtab_find(strtok, &mudstate_hot.flags_htab);
   }
   if ( i_type == IS_TYPE_TOGGLE ) {
      tp = (TOGENT *)ohtab_find(strtok, &mudstate_hot.toggles_htab);
   }
...
void 
totem_handle_error(int i_error, dbref player, char *s_type, char *s_inbuff) 
{
...
int 
totem_info(char *totem, char *s_buff)
{
...
int 
totem_perms(char *totem, int totem_type, char *totem_perms, dbref player)
{
   int i_rettype;
...
int 
totem_letter(char *totem, char totem_lett, int totem_tier) 
{
  char *lcname, *lcnp;
  TOTEMENT *hashp;
...
int 
totem_flags(char *s_target, dbref player, dbref cause, char *s_buff) 
{
   dbref target;
   char *fl1, *fl1p, *fl2, *fl2p, *fl3, *fl3p, *s_buffp;
   TOTEMENT *storedtag;
   ...
   fl1p = fl1 = alloc_lbuf("totem_flags_1");
   fl2p = fl2 = alloc_lbuf("totem_flags_2");
   fl3p = fl3 = alloc_lbuf("totem_flags_3");
   s_buffp = s_buff;
   for ( storedtag = (TOTEMENT *) hash_firstentry2(&mudstate.totem_htab, 1);
         storedtag;
         storedtag = (TOTEMENT *) hash_nextentry(&mudstate.totem_htab)) {
      /* If it's the default flag skip it and ignore -- this allows 'toggles' and 'non-flags' */
      if ( (storedtag->flaglett ==  '?') && (storedtag->flagtier == 0) ) {
         continue;
      }

      /* Skip if target can't see bit */
      if ( !totem_cansee_bit(player, target, storedtag->listperm) ) {
         continue;
      }

      if ( (dbtotem[target].flags[storedtag->flagpos] & storedtag->flagvalue) == storedtag->flagvalue ) {
         switch(storedtag->flagtier) {
            case 2: /* tier 2 flag letter */
               safe_chr(storedtag->flaglett, fl3, &fl3p);
               break;
            case 1: /* tier 1 flag letter */
               safe_chr(storedtag->flaglett, fl2, &fl2p);
               break;
            default: /* tier 0 flag letter */
   ...
   if ( *fl2 ) {
      safe_chr('[', s_buff, &s_buffp);
      safe_str(fl2, s_buff, &s_buffp);
      safe_chr(']', s_buff, &s_buffp);
   }
   if ( *fl3 ) {
      safe_chr('{', s_buff, &s_buffp);
      safe_str(fl3, s_buff, &s_buffp);
      safe_chr('}', s_buff, &s_buffp);
   }
   free_lbuf(fl1);
   free_lbuf(fl2);
...
void 
do_totem(dbref player, dbref cause, int key, char *flag1, char *flag2)
{
