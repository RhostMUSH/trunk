...
/* Column Width for initial 3 column topic index generated by mkhtml.
 * 26 is the default based on column width, but several topics exceed this.
 * 32 is the minimum for fully clean topic index generation.
 */
...
int main(int argc, char *argv[])
{
    ...
    for( topicidx = 0; topicidx < ntopics; topicidx++ ) {
      ...
      if( ++columns == 3 ) {
        ...
        if ( ckval > COLWIDTH )
           ckval = COLWIDTH;
        for( idx = 0; idx < COLWIDTH - ckval; idx++ ) { 
          fprintf(hfp, " ");
        }
      }
    }
