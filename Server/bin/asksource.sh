...
echo "[${X[31]}] 31. Max PID Queues     [${X[32]}] 32. Max Player Length  [${X[33]}] 33. Max Connected Players" 
            ...
            case "$1" in
               ...
               33) # modify max descriptors (i.e. connected players) here.  
                   ...
                   then
                      echo "Values are unchanged.  No changes will be applied.  Hit <RETURN> to continue."
                      read ANS
                   elif [ ${lc_conv} -lt 10 -o ${lc_conv} -gt 1000 ]
                   then
                      echo "Value is invalid.  Must be >= 10 or <= 1000  No changes will be applied.  Hit <RETURN> to continue."
                      read ANS
                   else
                      lc_value=${lc_conv}
                      echo "New value set to ${lc_conv}.  Hit <RETURN> to continue."
