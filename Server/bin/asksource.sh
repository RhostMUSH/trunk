...
if [ $(${MYGCC} -lpcre 2>&1|grep -e -c "(to find|cannot find)") -eq 0 ]
then
   X[25]="X"
fi
...
info() {
   ...
   case $INFOARG in
     ...
     26) echo "This option encrypts your passwords using a random seed and"
         ...
         ;;
     27) echo "This option enables the HSpace 4.2 space simulation libraries"
         ;;
     B*|b*) RUNBETA=1
         info $(echo $1|cut -c2-)
         RUNBETA=0
...
updatemakefile() {
   ...
   if [ "${X[27]}" = "X" ]
   then
      echo "Generating Makefile for HSpace libraries. Please wait..."
      echo "# HSpace 4.2" >> ../src/custom.defs
